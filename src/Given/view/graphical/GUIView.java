...
import view.graphical.actions.GraphicalKeyListener;
...
public class GUIView extends JFrame implements PawnsBoardView {
  ...
  private HumanViewActions viewActions;
  ...
  /**
   * Connects the GUI view with the controller.
   *
   * @param actions controller to be used with this view.
   */
  public void setActionsListener(HumanViewActions actions) {
   ...
   */
  public void updateHand(HumanViewActions viewActions) {
    handPanel.removeAll();
    cardPanels.clear();
    for (int i = 0; i < model.getPlayerHand(team).size(); i++) {
      Card c = model.getPlayerHand(team).get(i);
  ...
  private CardPanel getCardPanel(HumanViewActions controller, Card c, int i) {
    CardPanel panel = new CardPanel(c, i, team);
  ...
  /**
   * Update's the label of whose turn is it.
   */
  ...
  /**
   * Conventience method that calls all the other update methods. NOTE: updateHand is called with
  ...
  public void gameOver(PlayerTeam winner) {
    ...
    if (winner != null) {
      warnPlayer("game over. " + winner.toString() + " wins! gg");
    } else {
